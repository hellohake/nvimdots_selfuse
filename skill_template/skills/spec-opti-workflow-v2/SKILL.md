...
#### 3.A 初次生成模式
写 `spec-workflow-retro.md`（提案同级目录），遵循“输出格式 A”。AGENTS.md 部分只产 **Patch Plan + diff**，不直接改 `AGENTS.md`；每条必须写清 hard gate 结果、duplicate check、首次无 marker 时的插入策略、以及用户需要 approve/reject/rewrite 的决策点。
...
> 注：第六节仅在 `$2` 传入时生成；未传 `$2` 且用户确认跳过时，整节省略，并在回执里说明已按用户确认跳过输入侧复盘。

---

## 关键红线 (Hard Rules)
1. **只产 spec-workflow-retro.md + 可选 intake.md**：不写 gotchas/prompting、不落技能文件、不改 AGENTS.md。
2. **intake.md 是忠实快照（仅当用户提供原始提示词路径时）**：原样 copy，一字不改（含巨型 JSON）；只在源文档更新时刷新。
