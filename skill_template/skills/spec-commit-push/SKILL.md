...
3. **提案推断**：读阶段 0 定位到的 `proposal_dir` 下已存在的 `tasks.md`/`design.md`/`plan.md`/`spec-code-review.md`/`manual-test-commands.md` 提到的绝对路径或仓库路径加入；缺失的可选文件直接跳过，不要求模板预置；`proposal_dir` 所在 git repo 必须加入候选。
...
把"仓库 + 分支 + 待提交/待推送概况"列表给用户确认后进入阶段 1.2。找不到任何改动仓库 → 询问用户，不硬猜。

#### 阶段 1.2 · Review Readiness 预检（若存在 spec-code-review.md）
如果阶段 0 已定位 `proposal_dir`，检查 `<proposal_dir>/spec-code-review.md`：

1. 文件不存在：
   - 提示“未发现 spec-code-review.md，本次缺少 AI pre-commit review 记录”。
   - 默认不硬停；但在确认闸中把它列为风险项，让用户决定是否继续。轻量模板不要求预置该文件。
2. 文件存在：
   - 字段契约由邻居技能（`spec-code-review`）拥有；若实际结构与下述 `Review Run`/`CR Readiness`/`Fix Queue` 字段对不上 → **停手报告**，不臆测"视为通过"、不强行解析。
   - 只读取**最新一轮 `Review Run`**。
   - 解析 `CR Readiness`：
...
#### 阶段 1.3 · Human Decisions 预检（若存在 human-decisions.md）
...
2. 文件存在：
   - 字段契约由 `human-decisions.md` 拥有；若实际结构与 `Status`/`Blocking` 字段对不上 → **停手报告**，不臆测、不强行解析。
...
> 先自问：**为什么每次调用都 rebase `origin/master`？** master 高频迭代，攒着不 rebase，冲突越积越难解、MR 越难合；宁可每次小步对齐。
...
> 先自问两件事：**这条 message 给谁看？**（reviewer / 未来的 coding agent 快速看懂，不是技术方案书）**本轮是 MR 开篇还是增量？**（决定背景写全还是一句话带过）。
...
- 若阶段 1.2 存在 review readiness 风险（缺失 report、`PARTIAL`、用户要求跳过 `NO`），必须展示风险摘要，并要求额外短码 `r` 表示“已知 review readiness 风险，仍继续”。`r` 只确认 review 风险，不替代后续 `y/a/f`。
...
## push 前置预检清单（阶段 5 执行前逐项核对）

push 是本技能唯一对外不可逆动作。`git commit` 之后、`git push` 之前，对**每个仓库**逐项核对；任一项不通过 = 停在该仓、不 push、报告原因。这是操作层兜底清单，红线哲学见顶部「🔴 安全红线」，此处不重复。

- [ ] **rebase 已收口**：behind=0，或已基于 `origin/master` rebase 完成且不在 rebase 中途（无残留冲突标记）。
- [ ] **门禁已结**：阶段 1.2/1.3 的硬停项已清；遗留风险已在阶段 4 经 `r` 显式接受。
- [ ] **message 已确认**：本仓 message 已被用户 `y/a` 确认（若改过则重新确认）。
- [ ] **暂存区干净**：仅 `add` 本次相关改动，无夹带无关脏文件。
- [ ] **保护分支闸（独立复检）**：`git rev-parse --abbrev-ref HEAD` 与 `@{push}` 解析出的远端目标分支均不在黑名单（防 `push.default` 漏推 master）。
- [ ] **force 路径**：仅当本仓标记"需 force"——已给用户看过本地 vs 远端差异并拿到单字符 `f`，命令为 `git push --force-with-lease`；普通仓不得进入 force 路径。
