...
- `$1`：提案名或绝对路径（必填）。
- `$2`：**原始提示词文档路径**（强提醒输入，预期默认传）。这是你写给 agent 的初始提示词/澄清文档（如 `0_biz_impl/<日期>-<需求名>.md`）。传入触发输入侧复盘（见阶段 1.5 / 阶段 2 第 6 维）；未传 / 跳过的门禁逻辑统一见**阶段 -1**（唯一权威定义，不在此重复）。
...
> **Happy path（先记住这条主干，再看下面的分阶段细节与末尾红线索引）**：
> 门禁查 `$2`（阶段 -1）→ 探测历史 retro 决定初次/增量（阶段 0）→ 定位提案目录并注入全景语料（阶段 1）→ 快照原始输入为 intake.md（阶段 1.5，仅当 $2）→ 按维度诊断 Gap（阶段 2）→ 读模板写/合并 retro（阶段 3）→ 回执给路径+摘要+下一步（阶段 4）。
> 分阶段小节是每一步的唯一权威定义；末尾红线索引只是导航指针。
...
2. **按需读取产物**（不整体内联大文件，做证据摘要）：
   ...
   - **重点读料**：`backup.md`（若存在，作为排错轨迹核心语料）；不存在时不要创建或硬停，改为读取 `$3...`、`troubleshoot.md`、`debug-report.md`、`spec-code-review.md` 等已有材料，并在报告里标注“未发现 backup.md，偏差归档材料不足”。
   - `specs/<子规约>/spec.md`。
3. 读 `$3...` 补充文件。
...
1. 若 `NO_INTAKE_CONFIRMED=true` → 打印降级提示“已按用户确认跳过输入侧复盘（未提供原始提示词文档路径）”，跳过本阶段与阶段 2 第 6 维。
...
3. 读取 intake.md 作为阶段 2 第 6 维的输入语料。

### 阶段 2：复盘与 Gap 诊断
对照 **Spec 最终意图** 与 **backup.md 排错轨迹**，从下列维度各扫一次，每维显式写“是否有可沉淀项”。

> **维度编号 = 模板章节号（一一对应，便于直接落位）**：第 1/2/3/4/6 维分别写入模板 `references/spec-workflow-retro-template.md` 的 `一/二/三/四/六` 节；模板 `五、Checklist` 不是独立扫描维，而是由第 1-4 维结论派生汇总，故维度列表里没有“第 5 维”。
1. **[代码风格偏好]**：从被反复调整的改动抽用户正/反例偏好。
   - 记录前先自问：**这是该用户跨需求都稳定的偏好（值得沉淀），还是本提案一次性的口味（丢弃）？** 只有能复用的才写。
2. **[AGENTS.md Patch Plan]**：只为 AGENTS.md 生成 always-on 的候选补丁计划，不直接改文件。
   ...
   - 🧨 **NEVER 把只在本业务域成立的规则当 `always_on` 塞进 AGENTS.md**（如“海镜房卡片要先打 tag”）：AGENTS.md 会被注入此后**每一个**需求的启动上下文，一条错晋升的规则会污染所有无关任务、诱导 agent 在不相干场景硬套，而且几乎没人回头删——它会**永久**跑偏。拿不准 always_on 就降级为 `Decision=skip_gotchas`，宁可漏晋升也不要错晋升。
   ...
   - **只产 patch/diff 不落盘**：基于真实 AGENTS.md 结构与 marker 状态生成最小 patch，无 marker 时优雅新增标准区域；锐利语气仅限本技能的诊断章节，**写入 AGENTS 的规则文字保持短、硬、中性**。
3. **[可复用资产候选]**：是否出现可复用的操作序列（固定步骤/固定诊断路径/固定清单）？**只描述候选**，不产出技能成品文件。
   - 记录前先自问：**这套步骤换个提案/换个人还会原样重跑吗？** 是 → 资产候选；只是这次的临时操作 → 不记。
4. **[架构思路遗漏]**：澄清期遗漏的边界、架构交叉影响、分层职责侵入。
   - 记录前先自问：**这是 always-on 的架构常识（边界/分层，下次任何需求都该前置），还是仅本业务域成立的 scenario-bound 取舍？** 前者才可能进 AGENTS（再过第 2 维 hard gate），后者标 `Decision=skip_gotchas` 交 `gotchas`。
6. **[输入侧 / 提示词质量]**（仅当 $2 传入）：对照 `intake.md`（你说了什么）↔ 已有偏差材料（优先 `backup.md`；没有则用 `troubleshoot.md`、`debug-report.md`、`spec-code-review.md`、`$3...`）做**根因归因打标**：
...
> - 第 6 维标注的“反复出现的提示词毛病” → `gotchas` 治理技能写入 `~/.agents/gotchas/prompting.md`（对应模板第六节）。
...
**MANDATORY - READ ENTIRE FILE**：在写入或合并 `spec-workflow-retro.md` 前，必须完整读取 `references/spec-workflow-retro-template.md`（约 114 行），从头到尾，**不要设任何 range 限制**。**Do NOT load** 任何其他文件（本目录当前只有这一个 reference；此约束是为未来新增 reference 时防过载保留）。
...
**禁止整体覆盖**：
- 🧨 **NEVER 用 Write 整文件重写已存在的 retro**（看似“重新生成更干净”）：retro 是按时间戳累积的复盘账本，整写会**静默抹掉**前几次执行沉淀的认知偏差、已审 patch 状态和 `<!-- added/updated -->` 溯源标记——这些丢了无法从本次语料重建。必须先读旧文件，再逐条 upsert。
...
## 关键红线 (Hard Rules) — 红线索引

> 红线的**完整定义在对应阶段（唯一权威）**，这里只做导航指针，避免重复展开稀释浓度。

| 红线（速记） | 权威定义位置 |
|---|---|
| 边界：只产 retro + 可选 intake，不写 gotchas/prompting、不落技能文件、不改 AGENTS.md | 角色与目标 |
| 门禁：未传 `$2` 先停下索要路径 | 阶段 -1 |
| intake.md 忠实快照、校验忽略首行 | 阶段 1.5 |
| 幂等不覆盖、历史只 upsert + 打标签 | 阶段 3.B |
| AGENTS.md 只产 diff 不落盘、规则短硬中性 | 阶段 2 第 2 维 / 模板二 |
| 禁止盲目 Glob、定位失败即问 | 输入解析规则 / 阶段 1.1 |
| 禁止长篇说教、每条 ≤2 句 Do/Don't | 阶段 3 |
| 回执只给路径 + 摘要 + 比例 + 下一步，不重复正文 | 阶段 4 |

**禁止捏造（本红线唯一出处）**：正反例与归因必须可追溯到 backup/intake/spec/补充材料，无据不写。
