...
报告必须单独给出 `CR Readiness`，帮助用户判断什么时候进入低成本人工 CR。它不是替用户拍板，而是把人工 review 时机显式化。判定原则：

- `YES` 只用于没有未解决 accepted 项、没有新的 Blocker/Major、human_decision 已明确范围、且轻量校验没有 changed-file 新错误的情况。
- `PARTIAL` 用于没有已知 Blocker，但存在 human_decision/spec-contract 风险、待人工确认项、或验证缺口的情况。
- `NO` 用于仍有 Blocker、未修 accepted 项、AI loop 未收敛、修复范围超出 Fix Queue、或 changed-file 校验缺失/失败的情况。
