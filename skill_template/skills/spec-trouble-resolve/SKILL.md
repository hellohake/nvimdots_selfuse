...
#### 0.2 NEVER / HARD STOP 速查
这些红线分散在后续阶段；如果上下文被压缩，优先保留这一组判断：

- NEVER 处理无法追溯到本轮 `troubleshoot.md` 条目或本轮明确口述问题的旧任务。
- NEVER 把增删字段/接口/配置/核心流转/跨仓契约伪装成 Type B 局部 bugfix。
- NEVER 在问题不清楚时搜索代码、提出修法或写文件；先补齐现象、期望、范围。
- NEVER broad test/build；targeted test 也必须先有 repo policy、用户指令或 Type D 理由。
- NEVER 覆盖、回退或反向修改用户未提交改动；冲突时停手确认。
- NEVER 为了 diagnostics 清零而改变业务语义或做无关重构。
- NEVER 创建游离 `backup.md`；归档只能锚定 `CHANGE_DIR`。
- NEVER 重写、清洗、排序、合并旧 `backup.md` 内容；只追加。
- NEVER 自动删除无法唯一切分的 `troubleshoot.md` 内容。
- HARD STOP：多仓或多提案目录时，若无法确定唯一 `CHANGE_DIR`，先让用户选择归档锚点。
---
...
3. **再 fallback**：只有当没有 `.openspec.yaml`、文件不可读、或字段/注释没有可用路径时，才按顺序 fallback：
   ...
   找不到 schema 或 fallback 结果时，输出判断模板：`layout_source=<.openspec.yaml|fallback|not_found>, candidate_roots=[...], selected_CHANGE_DIR=<...|unknown>`。
...
若不足三项，或只有“这里不对 / 有问题 / 帮我修一下 / 这个返回不符合预期”这类泛描述，必须立即停下来向用户追问，禁止进入 1.2、禁止搜索代码、禁止提出修法。
...
澄清提问模板（一次最多 3 个问题；先补齐现象、期望、范围。复现、关联 spec、影响面、用户手改关系只作为后续可选追问池，只有前三项已补齐但仍阻塞时再问）：
```
我需要先澄清以下几点再动手：
1. 现象：具体哪里不对？有没有错误返回 / 日志 / 结果样例？
2. 期望：你期望这里按哪条需求或设计表现？
...
**MANDATORY - READ ENTIRE FILE**：如果本轮创建或修改任何 `.go` 文件，进入诊断前读取 `references/go-diagnostics-protocol.md` 并照做。
...
**MANDATORY - READ ENTIRE FILE**：进入归档阶段前读取 `references/archive-protocol.md` 并照做。
