---
name: gotchas
description: 经验复用飞轮的【生产端·提炼厂】。每次需求复盘后【手动】运行一次，从提案的 backup.md、spec-workflow-retro.md 或用户补充材料蒸馏两类沉淀：①“会重复犯的坑”→ 项目级 .ai_doc/spec-workflow/gotchas.md 与全局 ~/.agents/gotchas/general.md；②“反复出现的提示词毛病”→ 全局 ~/.agents/gotchas/prompting.md。Use when 用户说“沉淀坑点 / 跑 gotchas / 蒸馏经验 / 更新坑点库 / 沉淀提示词复盘”，或 OpenSpec-style SDD 复盘后。
---
...
- `$1`：提案名或绝对路径（必填）。用于定位提案目录；若存在则读取 `backup.md` 与 `spec-workflow-retro.md`。
- `$2...`：可选补充材料路径。
- 若提案目录缺少 `backup.md` / `spec-workflow-retro.md`，不要要求模板预置；读取已有材料和 `$2...`。若所有材料都缺失，提示用户按需补充 `backup.md`、`spec-workflow-retro.md` 或其它复盘材料后再运行。
...
- `src`：来源锚点，格式 `backup.md#<提案名或时间戳>`（prompting 用 `spec-workflow-retro.md#<提案名>` 或 `intake.md#<段落>`），用于**来源去重**（同一来源重复跑不重复计 hits）。多来源用 `;` 分隔。
...
2. **只读必要语料**：已存在的 `backup.md`、`spec-workflow-retro.md`、`$2...`。
   - `backup.md` 不存在：不要创建，也不要硬停；记录“代码/规划偏差归档未启用或尚未产生”。
   - `spec-workflow-retro.md` 不存在：不要创建，也不要硬停；提示如需输入侧/流程复盘可先运行 `spec-opti-workflow-v2` 或直接提供补充材料。
   - 若三类语料全部为空：停止并提示用户按需提供材料，不做无来源蒸馏。
...
1. 库文件不存在则先创建：
   - 项目库创建最简骨架（不要依赖 hello-spec-v2 模板文件）：
     ```
     # 项目 Gotchas
